...
sub _build_poker_command {
  ...
  return {
    ...
    'table_opts' => [
      \&table_opts, { %{ $self->table_option }, table_id => 1, tour_id => 0 }
    ],
    'table_chat' =>
...
sub table_chips {
  ...
  if ( $opts->{chips} > $login->fetch_chips($director_id) ) {
    $response->[1]->{message} = 'Not enough chips.';
    $login->send($response);
    return;
  }

  $login->debit_chips( $director_id, $opts->{chips} );
  $player->chips( $player->chips + $opts->{chips} );

  my $re = {
    table_id    => $table->table_id,
    director_id => $director_id,
    balance     => $login->fetch_chips($director_id),
    chips       => $player->chips,
    chair       => $chair->index,
  };
...
sub _ring_snap {
  ...
  $login->send(
    [
      'ring_snap',
      [ map { $self->_fetch_table_opts($_) } values %{ $self->table_list } ]
    ]
  );
}
...
sub pick_game {
  ...
  my $o =
    { map { $_ => $table->$_ }
      qw(table_id director_id action button turn_clock time_bank watch_list game_over dealer_choices lobby_data)
    };
